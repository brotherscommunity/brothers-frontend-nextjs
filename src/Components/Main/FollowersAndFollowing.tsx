    ...
    return (
        <main className='mt-10 mb-20'>
            ...
            <header className="w-full h-[50px]">
                {data?.followers && <div className="flex items-center gap-20">
                    <div>
                        <button onClick={() => setState(FOLLOW_STATE.followers)} className={`max-sm:text-base sm:text-lg md:text-xl ${state === FOLLOW_STATE.followers ? "text-blue-600" : "text-black"} pl-8 font-semibold`}>
                            {data?.followers} Followers
                        </button>
                        {state === FOLLOW_STATE.followers && <hr className="border-2 border-blue-600 max-sm:w-[180px] sm:w-[210px] mt-2" />}
                    </div>
                    <div>
                        <button onClick={() => setState(FOLLOW_STATE.following)} className={`max-sm:text-base sm:text-lg md:text-xl ${state === FOLLOW_STATE.following ? "text-blue-600" : "text-black"} pl-8 font-semibold`}>
                            {data?.following} Following
                        </button>
                        {state === FOLLOW_STATE.following && <hr className="border-2 border-blue-600 max-sm:w-[180px] w-[210px] mt-2" />}
                    </div>
                </div>}
            </header>
